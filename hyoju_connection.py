...
hyoju_connection.py
────────────────────────────────────────────────────────────────
seoyun 프론트엔드(Flask) ↔ hyoju 백엔드(Supabase) 연결 브릿지

사용법:
  seoyun 브랜치 app.py에서 기존 report_bp 대신 이 파일을 등록하면 됨.
...
# ────────────────────────────────────────────────────────────────
#  /api/weekly-report  ← seoyun 템플릿이 그대로 호출하는 경로 유지
# ────────────────────────────────────────────────────────────────
@hyoju_bp.route('/api/weekly-report')
def weekly_report():
    """
    seoyun 템플릿이 기대하는 필드:
      nickname, total, top_topic, top_topic_cnt,
      top_category, top_category_cnt, top_day, top_day_cnt
    """
    ...
    # topics[] → subcategory 역할
    topic_counter = Counter()
    for c in contents:
        for t in (c.get("topics") or []):
            topic_counter[t] += 1

    top_topic, top_topic_cnt = (
        topic_counter.most_common(1)[0] if topic_counter else ('-', 0)
    )

    # content_type → category 역할
    type_counter = Counter()
    for c in contents:
        type_counter[c.get("content_type", "other")] += 1
    top_cat, top_cat_cnt = (
        type_counter.most_common(1)[0] if type_counter else ('-', 0)
    )
    ...
    top_day_idx, top_day_cnt = (
        day_counter.most_common(1)[0] if day_counter else (0, 0)
    )

    # AI 닉네임 — seoyun 스타일 그대로 유지
...
# ────────────────────────────────────────────────────────────────
#  /api/weekly-stats  ← seoyun 템플릿이 그대로 호출하는 경로 유지
# ────────────────────────────────────────────────────────────────
@hyoju_bp.route('/api/weekly-stats')
def weekly_stats():
    """
    seoyun 템플릿이 기대하는 필드:
      top_topics [{subcategory, cnt}],
      all_topics [{subcategory, cnt}],
      content_types [{content_type, cnt}],
      recent_categories [{category, cnt, latest}],
      daily_counts [{day, label, cnt}]
    """
    user_id  = request.args.get("user_id")
    contents = _fetch_week_contents(user_id)

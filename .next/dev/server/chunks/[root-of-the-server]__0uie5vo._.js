...
"[project]/app/api/collections/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(req) {
    ...
    const { data: collections, error } = await db.from('collections').select('id, name, emoji, content_count, is_user_renamed, last_accessed_at, created_at, updated_at').eq('user_id', userId).order('updated_at', {
        ascending: false
    });
    ...
    const enriched = await Promise.all((collections || []).map(async (col)=>{
        const { data: contents } = await db.from('contents').select('id, url, content_type, title, thumbnail_url, author, metadata, hashtags, topics, moods, collection_id, saved_at').eq('collection_id', col.id).eq('user_id', userId).order('saved_at', {
            ascending: false
        }).limit(10);
        return {
            ...col,
...
async function PATCH(req) {
    ...
    const db = admin();
    const { error } = await db.from('collections').update({
        name,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0uie5vo._.js.map
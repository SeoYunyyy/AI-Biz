...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      // 7. [게이미피케이션] 카테고리 카운트 업데이트 & 아이템 지급 체크
      // topics를 FastAPI에 전달 → 카테고리 매핑 → 5개 달성 시 pending 아이템 생성
      fetch(`${FASTAPI_URL}/gamification/process-content`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: userId, topics: analysis.topics }),
      }).catch((e) => console.error('[gamification] 아이템 체크 실패:', e));
      // 게이미피케이션 실패해도 저장은 정상 진행되도록 await 하지 않음

      // 8. 완료된 콘텐츠 수 확인 → 30개 이상이면 클러스터링

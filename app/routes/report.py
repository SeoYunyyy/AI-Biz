...
async def weekly_report(
    ...
    if start_date and end_date:
        ...
        prev_until = prev_end.isoformat() + "T23:59:59"
    else:
        until_dt = now - timedelta(days=7 * week_offset)
        since_dt = until_dt - timedelta(days=7)
        since, until = since_dt.isoformat(), until_dt.isoformat()
        prev_since = (since_dt - timedelta(days=7)).isoformat()
        prev_until = since_dt.isoformat()
    contents = await _fetch(user_id or None, since, until)
    total = len(contents)
...
async def weekly_stats(
    ...
    since_date = datetime.fromisoformat(since[:10])
    until_date = datetime.fromisoformat(until[:10])
    span = max((until_date - since_date).days + 1, 7)
    date_range = [(since_date + timedelta(days=i)).strftime('%Y-%m-%d') for i in range(span)]
    daily_counts = [
...
async def monthly_report(user_id: str = Query(DEFAULT_USER_ID), year: int = Query(0), month: int = Query(0)):
    now = datetime.utcnow()
    y = year or now.year

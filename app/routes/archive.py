        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            response = await client.get(
                ...
                params={
                    ...
                    "analysis_status": "eq.completed",
                    "select": "id,url,title,category,sub_category,one_line_summary,detailed_summary,content_type,topics,thumbnail_url,saved_at",
                    "order": "saved_at.desc",
                },
            )
            response.raise_for_status()
            ...
            "items": [
                {
                    ...
                    "summary": r.get("detailed_summary") or r.get("one_line_summary", ""),
...
async def items(
    ...
    params = {
        "user_id": f"eq.{user_id}",
        "analysis_status": "eq.completed",
        "select": "id,url,title,category,sub_category,one_line_summary,detailed_summary,content_type,topics,thumbnail_url,has_deadline,deadline_date,deadline_note,saved_at",
        "order": "saved_at.desc",
    }
    if category:

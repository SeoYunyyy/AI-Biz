...
(function () {
    ...
    try {
      if (mode === 'login') {
        ...
        if (email === 'test@keepit.local') {
          ...
          showError('비밀번호가 올바르지 않습니다. (dev@keepit)');
  ...
  // ── 레이어 0: 원거리 앰비언트 (따뜻한 베이지·브라운 톤) ──
  const layer0 = [
    { cx: 0.72, cy: 0.27, r: 0.52, color: [196, 160, 110], speed: 0.00010, phase: 0.0, alpha: 0.55, px: 0.08 },
    { cx: 0.46, cy: 0.68, r: 0.44, color: [210, 180, 140], speed: 0.00014, phase: 2.7, alpha: 0.45, px: 0.06 },
    { cx: 0.88, cy: 0.50, r: 0.36, color: [180, 140, 90],  speed: 0.00012, phase: 5.0, alpha: 0.50, px: 0.07 },
    { cx: 0.26, cy: 0.20, r: 0.30, color: [220, 195, 160], speed: 0.00018, phase: 1.2, alpha: 0.38, px: 0.05 },
  ];

  // ── 레이어 1: 중간 글로우 (브라운·카라멜 톤) ──
  const layer1 = [
    { cx: 0.68, cy: 0.33, r: 0.25, color: [160, 100, 40],  speed: 0.00026, phase: 0.4, alpha: 0.70, px: 0.16 },
    { cx: 0.78, cy: 0.57, r: 0.21, color: [140, 90, 30],   speed: 0.00030, phase: 3.0, alpha: 0.65, px: 0.18 },
    { cx: 0.53, cy: 0.45, r: 0.17, color: [200, 155, 90],  speed: 0.00033, phase: 1.6, alpha: 0.60, px: 0.20 },
    { cx: 0.39, cy: 0.59, r: 0.13, color: [170, 120, 60],  speed: 0.00037, phase: 4.3, alpha: 0.55, px: 0.13 },
    { cx: 0.91, cy: 0.17, r: 0.12, color: [190, 145, 80],  speed: 0.00041, phase: 1.9, alpha: 0.50, px: 0.22 },
  ];

  // ── 레이어 2: 핀포인트 하이라이트 (작고 따뜻함, 강한 패럴랙스) ──
  const layer2 = [
    { cx: 0.70, cy: 0.28, r: 0.068, color: [130, 75, 20],  speed: 0.00056, phase: 0.2, alpha: 0.80, px: 0.32 },
    { cx: 0.73, cy: 0.43, r: 0.052, color: [150, 95, 35],  speed: 0.00066, phase: 2.3, alpha: 0.75, px: 0.38 },
    { cx: 0.60, cy: 0.23, r: 0.042, color: [110, 65, 15],  speed: 0.00050, phase: 3.8, alpha: 0.70, px: 0.29 },
  ];
  ...
  function draw(ts) {
    ...
    bloom.addColorStop(0,    'rgba(180,120,50,0.10)');
    bloom.addColorStop(0.28, 'rgba(150,95,30,0.06)');
    bloom.addColorStop(0.60, 'rgba(100,60,15,0.02)');
    ...
    if (mouse.x > 0) {
      ...
      cg.addColorStop(0,   'rgba(90,48,2,0.06)');
      cg.addColorStop(0.5, 'rgba(90,48,2,0.02)');
  ...
  window.addEventListener('load', () => {
    ...
    gsap.timeline({ defaults: { ease: 'power3.out' } })
      ...
      .to('.brand-lead',     { opacity: 1, y: 0, duration: 0.7 }, '-=0.5')
      .to('.brand-desc',     { opacity: 1, y: 0, duration: 0.7 }, '-=0.4')
      .to('.brand-features', { opacity: 1, y: 0, duration: 0.7 }, '-=0.4')
